...
function init() {
  ...
  // Optimize simulation size for mobile
  ...
  const trailsMaterial = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uDecay: { value: 0.975 }, // Slightly slower decay for a little more persistence
  ...
  loadImage("/SidSurvive-/portrait_top.jpg", topTexture, topTextureSize);
  loadImage("/SidSurvive-/portrait_bottom.jpg", bottomTexture, bottomTextureSize);
  ...
  function loadImage(url, targetTexture, textureSizeVector) {
    ...
    img.onload = function () {
      ...
      // Optimize texture size for mobile devices
      ...
      newTexture.generateMipmaps = false; // Disable mipmaps for performance
  ...
  function animate() {
    ...
    // Very slight delay increase (75ms instead of 50ms)
    if (isMoving && performance.now() - lastMoveTime > 75) {
      isMoving = false;
    }
