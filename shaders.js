...
const displayFragmentShader = `
  ...
  vec2 getCoverUV(vec2 uv, vec2 textureSize) {
    ...
    // Use a smaller scale factor to show more of the image (especially the head)
    float scale = max(s.x, s.y) * 0.85; // Reduced scale to show more content
    
    vec2 scaledSize = textureSize * scale;
    
    // Center the image both horizontally and vertically to ensure head is visible
